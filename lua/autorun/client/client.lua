...
local lever = false
hook.Add('ContextMenuOpen', 'callMenuWidget', function()

    local frame = vgui.Create('DFrame')
    frame:SetSize(72, 72)
    frame:SetPos(ScrW()/18, 40)
    frame:SetTitle('')
    frame:ShowCloseButton(false)
    frame:SetDraggable(false)
    frame:MakePopup()
    frame.Paint = function(self, w, h)
        draw.RoundedBox(5, 0, 0, w, h, Color(68, 68, 68, 164))
    ...
    local border = vgui.Create('DFrame', frame)
    border:SetSize(72, 72)
    border:Center()
    border:SetTitle('')
    border:SetDraggable(false)
    border:ShowCloseButton(false)
    border.Paint = function(self, w, h)
        surface.SetDrawColor(255, 255, 255)
        surface.DrawOutlinedRect(0, 0, 72, 72, 3)
    end

    local icon = vgui.Create('DButton', frame)
	icon:SetText('')
	icon:SetSize(64, 64)
    icon:Center()
    icon:SetImage('addons/pump-pump/materials/widgetIcon/pump.png')
    icon.Paint = nil
    ...
    icon.DoClick = function()
        if lever == false then
            ...
            LocalPlayer():ChatPrint('Менюшка уже запущена!')
        end
    end

    hook.Add('ContextMenuClosed', 'closeMenuWidget', function()
        frame:Close()
        lever = false
    end)
